...
const ToDoList = () => {
    ...
    return (
        <>
            ...
            <div className="container p-5 my-5 bg-dark text-white main" style={{backgroundColor:"gray",borderRadius:"10px"}} >




                <input value={state} onChange={changeHandler} style={{ margin: 10, outline: "none", bordSerRadius: 5, padding: 5 }} className="input" placeholder="Ex: Excercise for 30 min"></input>

                {/* <button type="submit" onClick={submitHandler}>Add To Do</button> */}
                <button type="button" className="btn btn-light" onClick={submitHandler}style={{backgroundColor:"violet",border:"1px solid grey"}} >Create Plan</button>
                        ...
                        <div className="list" key={index} style={{ backgroundColor: index % 2 ? "#3498DB" : "#8E44AD", margin: "1%", padding: "1.5%", borderRadius: "5px" }}>
                            <li key={index} style={{ listStyle: "none" }}>{`${index + 1}. ${value}`}
                                <div className="btnContainer" style={{ display: "flex", float: "right", gap: "5%" }}>
                                    <button value={value} name={index} type="submit" className="btn btn-light" onClick={updateList} style={{ float: "right", marginBottom: "20px",backgroundColor:"violet",border:"1px solid grey" }}>Update</button>

                                    <button value={index} type="submit" className="btn btn-light" onClick={deleteList} style={{ float: "right", marginBottom: "20px",backgroundColor:"violet",border:"1px solid grey" }}>Delete</button>
                                </div>
                            </li>
